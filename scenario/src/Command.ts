        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (SLToken Eth Address) (... deploy slToken)`
              // could be used to deploy a slToken if it doesn't exist, but
              // since there is no SLToken, that check would raise (when we'd
